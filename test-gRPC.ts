...
async function main() {
    ...
    const byAmountIn = true; // Fixed input amount
    const amount = 100000000; // 1 SUI (MIST)
    const sqrtPriceLimit = '4295048016'; // Min price limit for a2b

    // Create a zero coin for the output placeholder (USDC)
    // We need to know the type of the output coin to create a zero balance of it
    ...
    const result = tx.moveCall({
        ...
        arguments: [
            ...
            tx.object(Inputs.SharedObjectRef({
                ...
                mutable: true,
            })),                                // arg1: Pool (Shared Object)
            swapCoin,                           // arg2: Coin<T0> (Using Gas coin as SUI input)
            zeroCoin,                           // arg3: Coin<T1> (Zero coin for output placeholder)
    ...
    const buildClient = new SuiClient({ url: SUI_GRPC_ENDPOINT});
